      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {courses.map((course) => (
            <Card key={course.id} className="group hover:shadow-xl transition-all duration-300 border-0 shadow-md">
              <div className="relative overflow-hidden rounded-t-lg">
                ...
                />
                <Badge className="absolute top-4 left-4 bg-[#3264C3] hover:bg-[#3264C3]">{course.level}</Badge>
              </div>

              <CardHeader className="pb-3">
                <CardTitle className="text-lg font-semibold text-gray-900 group-hover:text-[#3264C3] transition-colors">
                  {course.title}
                </CardTitle>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  {course.features.map((feature, index) => (
                    <div key={index} className="flex items-center gap-2 text-sm text-gray-600">
                      <div className="h-1.5 w-1.5 bg-[#3264C3] rounded-full"></div>
                      <span>{feature}</span>
                    </div>
                  ))}
                </div>
                ...
                <div className="pt-4 border-t">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-2xl font-bold text-[#3264C3]">{course.price}</span>
                  </div>
                  <Button className="w-full bg-[#3264C3] hover:bg-[#3264C3]/90 text-white group">
                    Đăng ký ngay
        ...
        <div className="text-center">
          <Button
            variant="outline"
            size="lg"
            className="border-[#3264C3] text-[#3264C3] hover:bg-[#3264C3] hover:text-white bg-transparent"
          >
            Xem tất cả khóa học
